...
router.post('/create', [security.checkToken, bodyParser.json({ type: 'application/json' })], function(req, res) {
  ...
  tableMessage.create(req.database.db, req.body.parentMessageId, req.body.messageTyp, req.body.latitude, req.body.longtitude, req.body.plusCode, req.body.message.replace(/\'/g,"''"), req.body.style, req.body.messageUserId, function (err) {
    if (err) {
      response.status = 500;
      response.error = err;
    } else {
