...
function createQueueOverloadedError(pendingCount, maxPendingRequests) {
  const error = new Error('Viator database queue overloaded');
...
function createDbConfigFromEnv() {
  const connectionString = process.env.VIATOR_DATABASE_URL || process.env.DATABASE_URL;
  const sslEnabled = String(process.env.VIATOR_DB_SSL || process.env.DB_SSL || '').toLowerCase() === 'true';
  const max = Number(process.env.VIATOR_DB_POOL_MAX || process.env.DB_POOL_MAX || 10);
  ...
  return {
    host: process.env.VIATOR_DB_HOST || process.env.DB_HOST || 'localhost',
    port: Number(process.env.VIATOR_DB_PORT || process.env.DB_PORT || 5432),
    database: process.env.VIATOR_DB_NAME || process.env.DB_NAME || 'messagedrop_viator',
    user: process.env.VIATOR_DB_USER || process.env.DB_USER || 'messagedrop',
    password: process.env.VIATOR_DB_PASSWORD || process.env.DB_PASSWORD || undefined,
    max: Number.isFinite(max) && max > 0 ? max : 10,
    ssl: sslEnabled ? { rejectUnauthorized: false } : undefined
  };
}
...
class PostgresCompat {
  constructor(config, logger = console) {
    ...
    this._maxPendingRequests = resolveMaxPendingRequests(
      process.env.VIATOR_DB_MAX_PENDING_REQUESTS || process.env.DB_MAX_PENDING_REQUESTS
    );
  }
    ...
    return this._trackTask(async () => {
      ...
      try {
        ...
        try {
          ...
          this._logger?.warn?.('Viator PostgreSQL rollback failed', rollbackErr?.message || rollbackErr);
        }
        throw err;
      } finally {
        client.release();
...
class Database {
  ...
  init(logger) {
    ...
    try {
      ...
      this.logger.info('Connected to the messagedrop Viator PostgreSQL database.');
    } catch (err) {
  ...
  close() {
    try {
      this.db?.close((err) => {
        ...
        this.logger.info('Close the Viator database connection.');
      });
    } catch (err) {
      this.logger.error(err?.message || err);
    }
