...
function databaseSettings() {
  const connectionString = process.env.OVERPASS_DATABASE_URL;
  if (connectionString) {
    const url = new URL(connectionString);
    const database = decodeURIComponent(url.pathname.replace(/^\//u, ''));
    if (!database) throw new Error('OVERPASS_DATABASE_URL must include a database name');
    const owner = decodeURIComponent(url.username || process.env.OVERPASS_DB_USER || 'messagedrop');
  ...
  const database = process.env.OVERPASS_DB_NAME || 'messagedrop_overpass';
  const owner = process.env.OVERPASS_DB_USER || 'messagedrop';
  return {
    database,
    owner,
    connection: {
      host: process.env.OVERPASS_DB_HOST || 'localhost',
      port: Number(process.env.OVERPASS_DB_PORT || 5432),
      database: process.env.OVERPASS_DB_MAINTENANCE_NAME || 'postgres',
      user: owner,
      password: process.env.OVERPASS_DB_PASSWORD || undefined,
      ssl: sslConfig()
    }
  };
}
...
function sslConfig() {
  return String(process.env.OVERPASS_DB_SSL || '').toLowerCase() === 'true'
...
if (require.main === module) {
  main().catch((error) => {
    process.stderr.write(`Could not create Overpass database: ${error.message}\n`);
    process.exitCode = 1;
  });
}
