...
test('creates the configured download directory recursively', async () => {
  const previous = process.env.OVERPASS_DOWNLOAD_DIR;
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'overpass-downloads-'));
  const expected = path.join(root, 'nested', 'downloads');
  try {
    process.env.OVERPASS_DOWNLOAD_DIR = expected;
    ...
    if (previous === undefined) delete process.env.OVERPASS_DOWNLOAD_DIR;
    else process.env.OVERPASS_DOWNLOAD_DIR = previous;
