...
async function main() {
  ...
  let staged = false;
  if (!options.jobId) {
    await callbackResult((callback) => tableGeodataPoi.createJob(database.db, {
      id: jobId,
  ...
  try {
    ...
    await tableGeodataPoi.stageVersion(database.db, {
      versionId,
      dataset: {
        ...dataset,
    ...
    if (dataset.supersedes?.length) {
      await callbackResult((callback) => tableGeodataPoi.deleteDatasets(database.db, dataset.supersedes, callback));
    }
    process.stdout.write(`Activated version ${versionId} with ${poiCount} local POIs for ${dataset.id}.\n`);
    try {
      await callbackResult((callback) => tableGeodataPoi.cleanupRetiredVersions(
        database.db,
        dataset.id,
...
function parseArguments(args) {
  const options = {
    ...
    force: false,
    categories: categoryNames(),
    keepVersions: Number(process.env.GEODATA_DATASET_RETIRED_VERSIONS || 0),
    jobId: null,
    subcategories: null
  };
