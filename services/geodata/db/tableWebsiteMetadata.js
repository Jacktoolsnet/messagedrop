const METADATA_TABLE = 'tableGeodataWebsiteMetadata';
const REFERENCE_TABLE = 'tableGeodataWebsiteReference';
const JOB_TABLE = 'tableGeodataMetadataJob';
...
function init(db, callback) {
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_geodata_website_metadata_due
    ...
    CREATE INDEX IF NOT EXISTS idx_geodata_website_reference_normalized
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_geodata_metadata_job_running
...
function discoverWebsites(db, callback) {
  db.all(`
    ...
    FROM tableGeodataPoiVersion p
    JOIN tableGeodataDatasetVersion v ON v.versionId = p.versionId AND v.status = 'active'
    WHERE p.payload #>> '{contact,website}' ~* '^https://'
  `, [], callback);
}

function hasReferences(db, callback) {
  db.get(`SELECT EXISTS(SELECT 1 FROM ${REFERENCE_TABLE} LIMIT 1) AS present`, [], (error, row) => {
    callback(error, error ? undefined : Boolean(row?.present));
  });
}

function ensureReferencesBatch(db, references, callback) {
  const rows = (references || []).filter((row) => row?.websiteUrl && row?.normalizedUrl);
  if (!rows.length) return callback?.(null);
  const payload = JSON.stringify(rows);
  db.run(`
    INSERT INTO ${METADATA_TABLE} (websiteUrl)
  ...
  `, [payload], (error) => {
    ...
    db.run(`
      INSERT INTO ${REFERENCE_TABLE} (websiteUrl, normalizedUrl)
      SELECT item->>'websiteUrl', item->>'normalizedUrl'
      FROM jsonb_array_elements(?::jsonb) item
      ON CONFLICT (websiteUrl) DO UPDATE SET
        normalizedUrl = EXCLUDED.normalizedUrl, updatedAt = CURRENT_TIMESTAMP
    `, [payload], callback);
  });
}
...
module.exports = {
  METADATA_TABLE, REFERENCE_TABLE, JOB_TABLE, init, recover, discoverWebsites, hasReferences,
  ensureReferencesBatch, listDue, get, ensure, ensureReference,
  markFetching, markSucceeded, markNoMetadata, markRetryableFailed, markPermanentFailed,
  createJob, setJobTotal, updateJob, completeJob, failJob, runningJob, listJobs, cleanupJobs
};
