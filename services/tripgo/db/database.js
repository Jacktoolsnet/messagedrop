const tableTripGoLocationsCache = require('./tableTripGoLocationsCache');
...
function createQueueOverloadedError(pendingCount, maxPendingRequests) {
  const error = new Error('TripGo database queue overloaded');
...
function createDbConfigFromEnv() {
  const connectionString = process.env.TRIPGO_DATABASE_URL || process.env.DATABASE_URL;
  const sslEnabled = String(process.env.TRIPGO_DB_SSL || process.env.DB_SSL || '').toLowerCase() === 'true';
  const max = Number(process.env.TRIPGO_DB_POOL_MAX || process.env.DB_POOL_MAX || 10);
  ...
  return {
    host: process.env.TRIPGO_DB_HOST || process.env.DB_HOST || 'localhost',
    port: Number(process.env.TRIPGO_DB_PORT || process.env.DB_PORT || 5432),
    database: process.env.TRIPGO_DB_NAME || process.env.DB_NAME || 'messagedrop_tripgo',
    user: process.env.TRIPGO_DB_USER || process.env.DB_USER || 'messagedrop',
    password: process.env.TRIPGO_DB_PASSWORD || process.env.DB_PASSWORD || undefined,
    max: Number.isFinite(max) && max > 0 ? max : 10,
    ssl: sslEnabled ? { rejectUnauthorized: false } : undefined
  };
}
...
class PostgresCompat {
  constructor(config, logger = console) {
    ...
    this._maxPendingRequests = resolveMaxPendingRequests(
      process.env.TRIPGO_DB_MAX_PENDING_REQUESTS || process.env.DB_MAX_PENDING_REQUESTS
    );
  }
    ...
    return this._trackTask(async () => {
      ...
      try {
        ...
        try {
          ...
          this._logger?.warn?.('TripGo PostgreSQL rollback failed', rollbackErr?.message || rollbackErr);
        }
        throw err;
      } finally {
        client.release();
...
class Database {
  ...
  init(logger) {
    ...
    try {
      ...
      tableTripGoLocationsCache.init(this.db);

      this.initTriggers();
      this.initIndexes();

      this.logger.info('Connected to the messagedrop TripGo PostgreSQL database.');
    } catch (err) {
  ...
  close() {
    try {
      this.db?.close((err) => {
        ...
        this.logger.info('Close the TripGo database connection.');
      });
    } catch (err) {
      this.logger.error(err?.message || err);
    }
