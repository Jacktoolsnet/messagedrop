const TABLE = 'tableTripGoLocationsCache';
...
function init(db) {
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_tripgo_locations_last_accessed
...
function cleanExpired(db, maxAgeDays, callback) {
  const days = Math.max(1, Math.round(Number(maxAgeDays) || 30));
  db.run(`DELETE FROM ${TABLE} WHERE lastAccessed < CURRENT_TIMESTAMP - (? * INTERVAL '1 day')`, [days], callback);
}
