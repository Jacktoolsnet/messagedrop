...
function createQueueOverloadedError(pendingCount, maxPendingRequests) {
  const error = new Error('sticker database queue overloaded');
...
class PostgresCompat {
  constructor(config, logger = console) {
    ...
    this._maxPendingRequests = resolveMaxPendingRequests(
      process.env.STICKER_DB_MAX_PENDING_REQUESTS || process.env.DB_MAX_PENDING_REQUESTS
    );
  }
    ...
    return this._trackTask(async () => {
      ...
      try {
        ...
        try {
          ...
          this._logger?.warn?.('sticker PostgreSQL rollback failed', rollbackErr?.message || rollbackErr);
        }
        throw err;
      } finally {
        client.release();
