import { applyHideSeekSearch, createMinefieldHideSeekGame, createPlacement } from './minefield-hide-seek';

describe('minefield hide and seek', () => {
  const placement=[0,2,4,18,20,22];
  it('rejects adjacent mines',()=>{expect(()=>createPlacement([0,1,4,18,20,22])).toThrow()});
  it('lets the seeker win after all safe cells were found',()=>{
    let game=createMinefieldHideSeekGame('x','o',placement);
    for(let index=0;index<36;index+=1)if(!game.rounds[0].mines[index])game=applyHideSeekSearch(game,'o',index)!;
    expect(game.phase).toBe('finished');
    expect(game.winnerUserId).toBe('o');
    expect(game.nextPlayerUserId).toBeNull();
  });
  it('ends the round as lost when every mine is hit first',()=>{
    let game=createMinefieldHideSeekGame('x','o',placement);
    for(const index of placement)game=applyHideSeekSearch(game,'o',index)!;
    expect(game.phase).toBe('finished');
    expect(game.rounds[0].lost).toBeTrue();
    expect(game.winnerUserId).toBe('x');
  });
});
