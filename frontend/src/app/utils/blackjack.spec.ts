...
describe('blackjack duel',()=>{
  ...
  it('reveals the result as soon as the second player stands',()=>{const xSecret=createBlackjackSecret(),oSecret=createBlackjackSecret(),game=createBlackjackGame('x','o','xdeck','xcommit','odeck','ocommit');const afterX=applyBlackjackAction(game,'x','stand',xSecret)!;const finished=applyBlackjackAction(afterX,'o','stand',oSecret)!;expect(finished.phase).toBe('finished');expect(finished.status).not.toBe('active');expect(finished.revealedPlayerXHand).not.toBeNull();expect(finished.revealedPlayerOHand).not.toBeNull()});
});
