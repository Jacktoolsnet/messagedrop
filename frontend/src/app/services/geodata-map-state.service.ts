...
import { GeodataPoi, GeodataViewport } from '../interfaces/geodata';
import { GeodataService } from './geodata.service';

// Map interactions can emit many viewport updates in quick succession. A
// slightly longer pause protects the public Geodata instance from requests
// that would be obsolete before their response arrives.
const REQUEST_DEBOUNCE_MS = 750;

@Injectable({ providedIn: 'root' })
export class GeodataMapStateService {
  private readonly geodata = inject(GeodataService);
  private readonly destroyRef = inject(DestroyRef);
  private readonly viewportState = signal<GeodataViewport | null>(null);
  private readonly poisState = signal<GeodataPoi[]>([]);
  ...
  constructor() {
    effect((onCleanup) => {
      ...
      const timer = setTimeout(() => {
        subscription = this.geodata.getNearby(viewport.bounds, viewport.categories)
  ...
  setViewport(viewport: GeodataViewport | null): void {
...
function normalizeViewport(viewport: GeodataViewport): GeodataViewport {
  // Stable grid-aligned boxes make nearby map movements reuse the same
  // database cache entry instead of hammering the public Geodata instance
