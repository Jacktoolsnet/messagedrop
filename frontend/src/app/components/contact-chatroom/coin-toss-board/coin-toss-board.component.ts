...
export class CoinTossBoardComponent{
 readonly game=input<CoinTossGame|null>(null);readonly currentUserId=input('');readonly disabled=input(false);readonly toss=output<void>();readonly animating=signal(false);private readonly feedback=inject(GameFeedbackService);
 constructor(){effect(onCleanup=>{const g=this.game(),user=this.currentUserId(),disabled=this.disabled();if(disabled||!g?.result||!user)return;const key=`coin-toss-animation:${user}:${g.gameId}:${g.moveNumber}`;if(this.wasSeen(key))return;this.animating.set(true);const finish=setTimeout(()=>{this.animating.set(false);this.markSeen(key);this.feedback.notifyCoinLanding()},1900);onCleanup(()=>clearTimeout(finish))})}
