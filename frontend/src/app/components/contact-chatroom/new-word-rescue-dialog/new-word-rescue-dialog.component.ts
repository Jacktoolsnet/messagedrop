...
export interface NewWordRescueDialogResult{solution:string;hint:string;category:string}
...
export class NewWordRescueDialogComponent{
 ...
 readonly solution=signal('');readonly hint=signal('');readonly category=signal('');readonly show=signal(false);
 valid(){return isValidWordRescueSolution(this.solution())}
 close(){this.ref.close()}
 submit(){if(!this.valid())return;this.ref.close({solution:normalizeWordRescueText(this.solution()),hint:this.hint().trim(),category:this.category().trim()})}
}
