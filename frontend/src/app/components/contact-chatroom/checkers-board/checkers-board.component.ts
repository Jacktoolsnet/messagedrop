...
@Component({selector:'app-checkers-board',standalone:true,templateUrl:'./checkers-board.component.html',styleUrl:'./checkers-board.component.css',changeDetection:ChangeDetectionStrategy.OnPush})export class CheckersBoardComponent{
 readonly feedback=inject(GameFeedbackService);readonly game=input<CheckersGame|null>(null);readonly currentUserId=input('');readonly selectionMode=input(false);readonly disabled=input(false);readonly action=output<CheckersAction>();readonly selected=signal<number|null>(null);readonly cells=Array.from({length:64},(_,i)=>i);
