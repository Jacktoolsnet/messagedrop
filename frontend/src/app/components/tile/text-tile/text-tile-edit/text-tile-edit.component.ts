...
export class TextTileEditComponent {
  ...
  readonly titleControl = new FormControl(this.data.tile.payload?.title ?? this.data.tile.label ?? 'Text', { nonNullable: true });
  ...
  save(): void {
    const title = this.titleControl.value.trim() || 'Text';
