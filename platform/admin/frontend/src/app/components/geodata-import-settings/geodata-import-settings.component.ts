...
export class GeodataImportSettingsComponent {
  ...
  load(): void {
    ...
    forkJoin({ settings: this.service.getSettings(), catalog: this.service.getCatalog() })
      ...
      .subscribe({
        ...
        error: () => this.showError(this.i18n.t('Could not load Geodata settings.'))
      });
  }
  ...
  countryDataset(datasets: GeodataImportCatalog['datasets']): GeodataImportCatalog['datasets'][number] | null {
    return datasets.find((dataset) => dataset.level === 'country') ?? null;
  }

  toggleCountry(datasets: GeodataImportCatalog['datasets'], enabled: boolean): void {
    ...
    this.service.updateSettings(payload)
      ...
      .subscribe({
        next: (response) => {
          ...
          this.messages.open(this.i18n.t('Geodata settings saved.'), undefined, { panelClass: 'snack-success', verticalPosition: 'top' });
        },
        error: () => this.showError(this.i18n.t('Could not save Geodata settings.'))
      });
  }
  ...
  stepLabel(job: GeodataImportJob): string {
  ...
  processedDetails(job: GeodataImportJob): string | null {
  ...
  private initialSubcategories(settings: GeodataImportSettings, category: string, catalog = this.catalog()): string[] {
    return Object.hasOwn(settings.subcategories ?? {}, category)
      ? [...(settings.subcategories[category] ?? [])]
      : [...(catalog?.categories[category] ?? [])];
  }

  private buildPayload(settings: GeodataImportSettings): GeodataImportSettings {
  ...
  private comparable(settings: GeodataImportSettings): unknown {
