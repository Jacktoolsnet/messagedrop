...
@Component({
  ...
  templateUrl: './warn-logs.component.html',
  styleUrls: ['./warn-logs.component.css'],
...
export class WarnLogsComponent implements OnInit {
  ...
  load(): void {
    ...
    this.logService.listWarnLogs(200, 0).subscribe({
      next: (res) => {
      ...
      error: () => {
        this.snack.open(this.i18n.t('Could not load warn logs.'), this.i18n.t('OK'), { duration: 3000 });
      },
  ...
  deleteSelected() {
    ...
    this.logService.deleteWarnLog(current.id).subscribe({
      next: (res) => {
        if (res.deleted) {
  ...
  deleteAll() {
    ...
    const ref = this.dialog.open(ConfirmDialogComponent, {
      data: {
        title: 'Delete all warn logs?',
        message: 'Do you really want to delete all warn log entries? This cannot be undone.',
        confirmText: 'Delete',
        cancelText: 'Cancel',
        warn: true
      }
    ...
    ref.afterClosed().subscribe(confirmed => {
      if (!confirmed) return;
      this.loading.set(true);
      this.logService.deleteAllWarnLogs().subscribe({
        next: (res) => {
          if (res.deleted) {
            this.logs.set([]);
